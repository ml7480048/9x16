...
**День 16 Ранок + беклог (а)+(б) закриті (2026-07-02, одна сесія):** (1) **Анімації** за правилом дизайн-системи "subtle fade only": fade-in на зміні кроку wizard (keyed wrapper), на перемиканні A/B/C варіантів (клас на unhide — без ремаунту, preload живий), на sessions list/detail. (2) **Per-lead квоти** (беклог (а)): таблиця `usage_events` у Postgres, ліміти per-email у `db.ts` (video 30 / image 120 / scenes 60 / script 60 — відео найдорожчі), `enforceQuota()` у всіх 4 генераційних роутах, 429 з чітким повідомленням при перевищенні; без POSTGRES_URL (локальний dev) квоти пропускаються; помилка перевірки квоти fail-open (бухгалтерія не має класти продукт). Скидання квоти = видалити рядки з usage_events руками. (3) **Vercel Blob** (беклог (б)): `src/lib/storage.ts` — після генерації сервер копіює Kling-ассет у Blob і повертає наш постійний URL (generate-image, generate-video, check-video-status); копія time-boxed 25с щоб не пробити maxDuration; будь-яка помилка → fallback на Kling URL (генерація ніколи не втрачається через сховище). **Ручний крок для Маріана: у Vercel Dashboard → Storage → Create Blob store і підключити до проєкту** — токен BLOB_READ_WRITE_TOKEN приїде сам; без нього все працює як раніше (URL протухають). `mediaLikelyExpired` тепер попереджає лише якщо в сесії лишились klingai.com URL-и. Перевірено: tsc/eslint/build чисто, gate 401/пропуск живим тестом; квоти й Blob наживо перевіряться після деплою+підключення стора.
...
| День | Сесія | Задача | Статус |
|---|---|---|---|
| 16 | Ранок | Animations + transitions | ✅ Done |
